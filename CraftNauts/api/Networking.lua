--[[
    The Networking API runs as a coroutine waiting until it is needed. When needed it acts as a medium between the internet-based server and the game

    @version 1.0, 5 October 2013, BIT
    @author  TheOriginalBIT, BIT
--]]

assert(Log, "Log should be loaded before Networking", 0)

Log.i("Networking init!")
...
local function handleCommunication(url, callback, data)
  ...
  -- wait for a response
  local success, response = waitForHttpResponse()
...
local function run()
  while true do
    ...
    if url == "die" then
      return Log.i("Shutting down Networking routine")
    end
